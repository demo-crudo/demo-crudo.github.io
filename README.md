...
`/challenge` must provide ALTCHA PBKDF2/SHA-256 challenges. Both registration and sign-in require a verified ALTCHA proof; the browser component supplies its Base64 JSON `altcha` form value in the JSON request body. Challenges are one-time and reset after every submission. The app loads the official pinned ALTCHA browser component from `https://cdn.jsdelivr.net/npm/altcha@3.2.1/+esm`, so this CDN must be reachable (or be permitted by the applicable content-security policy).
